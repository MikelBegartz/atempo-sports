...
- **Format uniforme**: ja no hi ha línia pròpia "Tot l'equip" (`team_shared_all` eliminada del detector) — mentia quan l'equip tenia poques persones fitxades (p.ex. només l'entrenador: "tot l'equip" amb 1 persona). Ara tot és `A ⇄ B — N persones`; si la plantilla sencera està duplicada (≥3 persones), s'afegeix el sufix "· tot l'equip X" (`_overlap_full_team` + clau `cgrp_full_team`). Variants singulars `*_1` per a "1 persona".
- **Text amb rols** (`_group_who`): el resum diu qui és afectat — "1 entrenador", "12 jugadors", "2 jugadors, 1 reforç" — en lloc de "N persones" (claus `cgrp_overlap_who`, `cgrp_coach_gap_who`, `cgrp_role_*_pl`). Rol per persona = el de més pes als dos equips (coach > delegate > reinforce > player); ordre de lectura: jugadors, reforços, entrenadors, delegats. Fallback a "N persones" si no hi ha rols.
